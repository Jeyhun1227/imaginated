                    ...
                    {/* <div className="content-area" > */}
                        <div className="flex flex-row flex-wrap space-x-3">
                                ...
                                <div className="inline-block ml-2 no-underline text-dark-blue font-semibold cursor-point">
                                    <Link href='/shop/photoshop-actions/'><a>Photoshop Actions</a></Link>
                                    <div className="inline-flex pointing-right"><ChevronRight/></div>
                                </div>
...
export async function getStaticProps(context) {
    ...
    let metadata_raw = await axios.get(`https://wordpress.imaginated.com/wp-json/rankmath/v1/getHead?url=https://wordpress.imaginated.com/shop/photoshop-actions/${slug}`)
...
export async function getStaticPaths() {
    ...
    let shopKeysClean = []
    shopKeys.filter((e) => {
        if(e.includes('photoshop-actions')){
            let shopValue = e.replace('/shop/photoshop-actions/', '').split('/')[0];
